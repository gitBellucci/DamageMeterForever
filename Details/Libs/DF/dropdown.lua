...
-- Forever / clients where GameFont*:GetFont() can return a missing path.
-- SetFont requires a file path; SetFontObject accepts a Font object.
local FALLBACK_FONT_FILE = "Fonts\\FRIZQT__.TTF"
do
	...
	if ok and type(path) == "string" and path ~= "" and not path:find("GameFont", 1, true) then
		FALLBACK_FONT_FILE = path
	end
end

local function SafeSetFont(fontString, fontFileOrObject, size, flags)
	...
	local function tryFile(path)
		if type(path) ~= "string" or path == "" or path:find("GameFont", 1, true) then
			return false
		end
		return pcall(fontString.SetFont, fontString, path, size, flags)
	end

	-- Font object passed directly
	if type(fontFileOrObject) == "table" and fontFileOrObject.GetFont then
		local okPath, path = pcall(fontFileOrObject.GetFont, fontFileOrObject)
		if okPath and tryFile(path) then
			return
		end
		if fontString.SetFontObject then
			pcall(fontString.SetFontObject, fontString, fontFileOrObject)
			return
		end
	end

	if tryFile(fontFileOrObject) then
		return
	end

	-- Resolve named GameFont* via GetFont, then fall back to Friz
	if type(fontFileOrObject) == "string" and _G[fontFileOrObject] and _G[fontFileOrObject].GetFont then
		local okPath, path = pcall(_G[fontFileOrObject].GetFont, _G[fontFileOrObject])
		if okPath and tryFile(path) then
			return
		end
	end

	if not tryFile(FALLBACK_FONT_FILE) then
		if fontString.SetFontObject and GameFontHighlightSmall then
			pcall(fontString.SetFontObject, fontString, GameFontHighlightSmall)
		end
	end
end
...
function DropDownMetaFunctions:Selected(thisOption)
	...
	if (overrideFont) then
		SafeSetFont(self.label, overrideFont, 10)

	elseif (thisOption.font and not self.isSimpleHeader) then
		SafeSetFont(self.label, thisOption.font, 10)

	else
		SafeSetFont(self.label, GameFontHighlightSmall, 10)
	end
		...
		if (optionsTable and optionsTable[1]) then
			...
			for tindex, thisOption in ipairs(optionsTable) do
				...
				if (bIsOptionVisible) then
					...
					if (overrideFont) then
						SafeSetFont(thisOptionFrame.label, overrideFont, 10.5)

					elseif (thisOption.font) then
						SafeSetFont(thisOptionFrame.label, thisOption.font, 10.5)

					else
						SafeSetFont(thisOptionFrame.label, GameFontHighlightSmall, 10.5)
					end
