...
`DF.folder` is computed once from `debugstack` and stores the full path to the framework root (`Interface\AddOns\DamageMeterForever\Libs\DF\` for the bundled copy). Use `DF:GetFrameworkFolder()` to read it.
...
8. **The framework caches `DF.folder` from `debugstack`.** Don't recommend hardcoding `Interface\AddOns\DamageMeterForever\Libs\DF\` in a consumer — use `DF:GetFrameworkFolder()`.
