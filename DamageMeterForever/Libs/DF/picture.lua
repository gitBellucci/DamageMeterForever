		...
		if (texture) then
			if (type(texture) == "table") then
				if (texture.gradient) then
					...
					if (detailsFramework.IsDragonflightAndBeyond() or detailsFramework.IsNonRetailWowWithRetailAPI() or detailsFramework.IsForeverWow() or detailsFramework.IsAddonApocalypseWow() or (ImageObject.image.SetGradient and CreateColor and not ImageObject.image.SetGradientAlpha)) then
						...
						if CreateColor then
							fromColor = CreateColor(fromColor.r or 0, fromColor.g or 0, fromColor.b or 0, fromColor.a or 1)
							toColor = CreateColor(toColor.r or 0, toColor.g or 0, toColor.b or 0, toColor.a or 1)
						end
						...
						if ImageObject.image.SetGradientAlpha then
							ImageObject.image:SetGradientAlpha(gradientTable.gradient, fromR, fromG, fromB, fromA, toR, toG, toB, toA)
						elseif ImageObject.image.SetGradient and CreateColor then
			...
			elseif (type(texture) == "string") then
				local isAtlas = C_Texture and C_Texture.GetAtlasInfo and C_Texture.GetAtlasInfo(texture)
				if (isAtlas) then
...
function detailsFramework:SetTexture(object, texture)
	...
	if (type(texture) == "table") then
		...
		elseif (textureInfo.gradient) then
			...
			if (detailsFramework.IsDragonflightAndBeyond() or detailsFramework.IsNonRetailWowWithRetailAPI() or detailsFramework.IsForeverWow() or detailsFramework.IsAddonApocalypseWow() or (object.SetGradient and CreateColor and not object.SetGradientAlpha)) then
				...
				elseif object.SetGradient and CreateColor then
					object:SetGradient(gradientTable.gradient, CreateColor(fromR, fromG, fromB, fromA), CreateColor(toR, toG, toB, toA))
				end
				return
			end
		else
