...
local isClassicEra = function()
	-- Forever (TOC 16xxx) is not Classic Era talent UI; treat like modern client.
	if (buildInfo >= 16000 and buildInfo < 20000) then
		return false
	end
...
local gameVersionHasMythicPlus = function()
    return C_ChallengeMode and C_ChallengeMode.ClearKeystone and true
end
...
do --> talents
        ...
        for tabIndex = 1, 3 do
            ...
            for i = 1, (MAX_NUM_TALENTS or 20) do
                if (i <= numTalents) then
                    ...
                    talentInfoQuery.groupIndex = PlayerTalentFrame and PlayerTalentFrame.talentGroup

                    ---@type classictalentinfo
                    local talentInfo = C_SpecializationInfo and C_SpecializationInfo.GetTalentInfo and C_SpecializationInfo.GetTalentInfo(talentInfoQuery)
    ...
    getTalents = function()
        ...
        if isClassicEra() then
            if PlayerTalentFrame_LoadUI then
                PlayerTalentFrame_LoadUI()
            end
        else
            local addonName, title, notes, canLoad, reasonCantLoad
            if C_AddOns and C_AddOns.GetAddOnInfo then
                addonName, title, notes, canLoad, reasonCantLoad = C_AddOns.GetAddOnInfo("Blizzard_TalentUI")
            end
            --tbc / modern
            if reasonCantLoad ~= "MISSING" and TalentFrame_LoadUI and C_AddOns and not C_AddOns.IsAddOnLoaded("Blizzard_TalentUI") then
                TalentFrame_LoadUI()
            end
        end
