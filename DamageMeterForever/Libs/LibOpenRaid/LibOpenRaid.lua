...
--internals
    ...
    function openRaidLib.CooldownManager.OnPlayerCast(event, spellId, isPlayerPet) --~cast
        ...
        -- Forever / Era DB may never define this global; keep an empty table.
...
local createLocalCooldownTracker = function()
    ...
    cdTrackerFrame:SetScript("OnEvent", function(self, event, ...)
        if (event == "UNIT_SPELLCAST_SUCCEEDED") then
            ...
            -- Forever: secrets cannot be used as keys or in boolean tests.
            if (issecretvalue and (issecretvalue(spellId) or issecretvalue(castGUID) or issecretvalue(unitId))) then
                return
            end
            if (type(spellId) ~= "number") then
                return
            end
            -- Skip odd unit tokens (e.g. targettarget) and player; UnitIsUnit can return a secret boolean.
            if (type(unitId) ~= "string") then
                return
            end
            if (unitId == "player" or unitId == "target" or unitId == "targettarget" or unitId == "mouseover" or unitId == "focus") then
                return
            end
            if (not unitId:find("^party%d") and not unitId:find("^raid%d")) then
                return
            end

            --get the caster name and check if it's a unit in the group
            local casterName = GetUnitName(unitId, true)
            if (casterName and not (issecretvalue and issecretvalue(casterName))) then
                local inParty = UnitInParty(unitId)
                local inRaid = UnitInRaid(unitId)
                if (issecretvalue and (issecretvalue(inParty) or issecretvalue(inRaid))) then
                    return
                end
                if (inParty or inRaid) then
                        ...
                        if (spellData) then
                            ...
                            if (not unitCastCooldown[spellId] or unitCastCooldown[spellId]+5 < GetTime()) then
                                ...
                                -- buff duration can also be secret
                                if (issecretvalue and issecretvalue(buffDuration)) then
                                    buffDuration = duration
                                end
                                if (issecretvalue and (issecretvalue(timeLeft) or issecretvalue(duration))) then
                                    return
                                end
