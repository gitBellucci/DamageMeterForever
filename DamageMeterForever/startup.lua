...
function Details222.StartUp.StartMeUp()
	...
	C_Timer.After(3, function()
		if Details.FillUserCustomSpells then
	...
	Details.MicroButtonAlert.Text = Details.MicroButtonAlert:CreateFontString(nil, "overlay")
	pcall(Details.MicroButtonAlert.Text.SetFont, Details.MicroButtonAlert.Text, "Fonts\\FRIZQT__.TTF", 12)
	Details.MicroButtonAlert.Text:SetTextColor(1, 0.82, 0, 1)
	...
	if (Details.is_version_first_run) then
		...
		if Details.AddDefaultCustomDisplays then
			Details:AddDefaultCustomDisplays()
		end
	end
	...
	--open welcome — Forever uses a short Foofi splash, not the stock Details wizard
	if (Details.is_first_run) then
		C_Timer.After(1, function()
			if (DetailsFramework.IsForeverWow and DetailsFramework.IsForeverWow()) then
				Details:OpenDamageForeverWelcome()
			else
				Details:OpenWelcomeWindow()
			end
		end)
	...
	local customSpellList = Details.GetDefaultCustomItemList and Details:GetDefaultCustomItemList() or {}
