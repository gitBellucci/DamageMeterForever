...
--damage taken by spell
	...
	function Details:ToolTipBySpell(instance, tabela, thisLine, keydown)
		...
		--GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\key_shift]], 1, 2, Details.tooltip_key_size_width, Details.tooltip_key_size_height, 0, 1, 0, 0.640625, Details.tooltip_key_overlay2)
...
--frags
				...
				for i = 1, math.min(min, #damage_taken_table) do
					...
					if (classe == "UNKNOW") then
						...
						if (specID) then
							...
							GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\classes_small_alpha]], nil, nil, iconSize, iconSize, unpack(Details.class_coords [classe]))
						end
					end
	...
	function damageClass:AtualizarFrags(tabela, whichRowLine, colocacao, instancia)
		...
		if (tabela [3] == "UNKNOW" or tabela [3] == "UNGROUPPLAYER" or tabela [3] == "ENEMY") then
			thisLine.icone_classe:SetTexture([[Interface\AddOns\DamageMeterForever\images\classes_plus]])
...
-- Forever TOC is 16xxx (< 120000) but uses the same C_DamageMeter API as Midnight.
local tocVersion = select(4, GetBuildInfo())
local oldapi = tocVersion < 120000 and not (tocVersion >= 16000 and tocVersion < 20000)
function Details:IsUsingBlizzardAPI(instance)
	if oldapi then
		return false
	end
	if DetailsFramework and DetailsFramework.IsForeverWow and DetailsFramework.IsForeverWow() then
		-- Forever always feeds from Blizzard's damage meter (no CLEU).
...
function Details222.Apocalypse.ShouldUseGameData(instance)
	if detailsFramework.IsAddonApocalypseWow() then
		-- Forever / Midnight: always read Blizzard C_DamageMeter (no CLEU fallback).
...
function Details:SetClassIcon(texture, instance, class) --[[exported]] --~icons
	...
	elseif(class == "UNKNOW") then
		texture:SetTexture([[Interface\AddOns\DamageMeterForever\images\classes_plus]])
	...
	elseif(class == "UNGROUPPLAYER") then
		...
		if (englishClass) then
			texture:SetTexture(instance.row_info.icon_file or [[Interface\AddOns\DamageMeterForever\images\classes_small]])
	...
	elseif(class == "PET") then
		texture:SetTexture(instance and instance.row_info.icon_file or [[Interface\AddOns\DamageMeterForever\images\classes_small]])
			...
			if (prescienceData) then
				...
				for amountOfPrescienceApplied, time in ipairs(prescienceData) do
					if (time > 0) then
						...
						GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\spells\prescience_time]], nil, nil, iconSize, iconSize)
	...
	do
		...
		--MOSTRA HABILIDADES
			...
			if (is_maximized) then
				...
				--GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\key_shift]], 1, 2, Details.tooltip_key_size_width, Details.tooltip_key_size_height, 0, 1, 0, 0.640625, Details.tooltip_key_overlay1)
		...
		--targets(enemies)
			...
			if (instance.sub_atributo == 1 or instance.sub_atributo == 6) then
				...
				Details:AddTooltipSpellHeaderText(Loc ["STRING_TARGETS"], headerColor, #ActorTargetsSortTable, [[Interface\AddOns\DamageMeterForever\images\icons]], 0, 0.03125, 0.126953125, 0.15625)
				...
				if (is_maximized) then
					--highlight
					GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\key_ctrl]], 1, 2, Details.tooltip_key_size_width, Details.tooltip_key_size_height, 0, 1, 0, 0.640625, Details.tooltip_key_overlay2)
					Details:AddTooltipHeaderStatusbar(r, g, b, 1)
				else
					GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\key_ctrl]], 1, 2, Details.tooltip_key_size_width, Details.tooltip_key_size_height, 0, 1, 0, 0.640625, Details.tooltip_key_overlay1)
					Details:AddTooltipHeaderStatusbar(r, g, b, barAlha)
				end
		...
		for index, damageTable in ipairs(petDamageTable) do
			if (damageTable [2] > 0 and(index <= Details.tooltip.tooltip_max_pets or ismaximized)) then
				if (not petHeaderAdded) then
					...
					if (ismaximized) then
						GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\key_alt]], 1, 2, Details.tooltip_key_size_width, Details.tooltip_key_size_height, 0, 1, 0, 0.640625, Details.tooltip_key_overlay2)
						Details:AddTooltipHeaderStatusbar(r, g, b, 1)
					else
						GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\key_alt]], 1, 2, Details.tooltip_key_size_width, Details.tooltip_key_size_height, 0, 1, 0, 0.640625, Details.tooltip_key_overlay1)
						Details:AddTooltipHeaderStatusbar(r, g, b, barAlha)
					end
				end
				...
				GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\classes_small_alpha]], 1, 1, icon_size.W, icon_size.H, 0.25/2, 0.49609375/2, 0.75/2, 1/2)
			end
		end
	end
...
function damageClass:ToolTip_DamageTaken(instance, numero, barra, keydown)
	...
	if (subAttribute == DETAILS_SUBATTRIBUTE_ENEMIES) then
		...
		--Details:AddTooltipSpellHeaderText(Loc ["STRING_FROM"], headerColor, #damageTakenDataSorted, [[Interface\AddOns\DamageMeterForever\images\icons]], 0.126953125, 0.1796875, 0, 0.0546875)
	end
...
function damageClass:MontaInfoDamageDone()
		...
		if (petActor) then --PET
			...
			if (bShouldMergeSpellsWithThePet and petSpellContainer:HasTwoOrMoreSpells()) then
				...
				local bkSpellData = {
					...
					bCanExpand = true,
					actorIcon = [[Interface\AddOns\DamageMeterForever\images\pets\pet_icon_1]],
				}
				detailsFramework:Mixin(bkSpellData, Details.SpellTableMixin)
	...
	if (instance.sub_atributo == DETAILS_SUBATTRIBUTE_ENEMIES) then
		...
		for index, thisEnemyTable in ipairs(enemyTable) do
			...
			thisLine.icone:SetTexture([[Interface\AddOns\DamageMeterForever\images\classes_small_alpha]]) --class icon
		...
		for index, targetTable in ipairs(allActorTargets) do
			...
			if (targetActorObject) then
				...
				barra.icone:SetTexture([[Interface\AddOns\DamageMeterForever\images\classes_small_alpha]]) --CLASSE
...
--[[
	...
	if (enemy) then
		...
		GameCooltip:AddIcon([=[Interface\AddOns\DamageMeterForever\images\key_shift]=], 2, 2, Details.tooltip_key_size_width, Details.tooltip_key_size_height, 0, 1, 0, 0.640625, Details.tooltip_key_overlay2)
