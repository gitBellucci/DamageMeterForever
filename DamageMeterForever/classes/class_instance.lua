...
function Details:PostponeSwitchToCurrent(instance)
	if (
		...
		if (instance.segmento > 0 and instance.auto_current) then
			...
			instance:InstanceAlert(Loc ["STRING_CHANGED_TO_CURRENT"], {[[Interface\AddOns\DamageMeterForever\images\toolbar_icons]], 18, 18, false, 32/256, 64/256, 0, 1}, 6)
			return
		else
			return
		end
...
function Details:MontaAtributosOption (instancia, func)
	...
	for i = 1, atributos[0] do --[0] armazena quantos atributos existem
		...
		if detailsFramework.IsAddonApocalypseWow() then
			...
			for o = 1, atributos [i] do
				...
				if (Details:CaptureIsEnabled ( Details.atributos_capture [gindex] ) and type(damageMeterType) == "number" and damageMeterType < 100) then
	...
	CoolTip:SetOption("StatusBarTexture", [[Interface\AddOns\DamageMeterForever\images\bar4_vidro]])
