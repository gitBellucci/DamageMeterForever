...
local statusBarBackgroundTable_ForDeathTooltip = {
	value = 100,
	texture = [[Interface\AddOns\DamageMeterForever\images\bar_serenity]],
...
function Details.ShowDeathTooltip2(instance, lineFrame) --~death
	...
	for i = #events, 1, -1 do
		...
		GameCooltip:SetOption("StatusBarTexture", [[Interface\AddOns\DamageMeterForever\images\bar_hyanda]])
	end
...
function Details.ShowDeathTooltip(instance, lineFrame, combatObject, deathTable) --~death
	...
	if (lastcooldown) then
		...
		gameCooltip:AddStatusBar(0, 1, 1, 1, 1, 1, false, {value = 100, color = {.3, .3, .3, 1}, specialSpark = false, texture = [[Interface\AddOns\DamageMeterForever\images\bar_serenity]]})
	end
...
function atributo_misc:ToolTipCC(instancia, numero, barra)
	...
	for _spellid, _tabela in pairs(habilidades) do
		...
		for target_name, amount in pairs(_tabela.targets) do
			...
			GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\espadas]], nil, 1, lineHeight, lineHeight)
			if (classe) then
				GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\classes_small]], nil, 2, lineHeight, lineHeight, unpack(Details.class_coords [classe]))
			else
	...
	if (#meus_pets > 0) then --teve ajudantes
		...
		for index, _table in ipairs(totais) do
			...
			if (_table [2] > 0 and (index < 3 or ismaximized)) then
				...
				Details:AddTooltipBackgroundStatusbar()
				GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\classes_small]], 1, 1, 14, 14, 0.25, 0.49609375, 0.75, 1)
			end
		end
