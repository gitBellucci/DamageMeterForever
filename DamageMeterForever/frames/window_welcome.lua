	...
	if (not window) then
		...
		window:SetScript("OnHide", function()
			...
			if (DetailsFramework.IsClassicWow()) then
				...
				new_instance.row_info.icon_file = [[Interface\AddOns\DamageMeterForever\images\classes_small]]
		...
		logotipo:SetPoint("topleft", window, "topleft", 16, -16)
		logotipo:SetTexture([[Interface\AddOns\DamageMeterForever\images\damageforever_logo]])
		logotipo:SetTexCoord(0, 1, 0, 1)
		logotipo:SetWidth(280)
		logotipo:SetHeight(88)
...
--page 1
		...
		-- Foofi: no stock Details angel art
		local angel = window:CreateTexture(nil, "border")
		angel:SetPoint("bottomright", window, "bottomright", -8, 8)
		angel:SetColorTexture(0.047, 0.824, 0.616, 0.06)
		angel:SetWidth(280)
		angel:SetHeight(200)
		angel:Hide()
...
--Skins Page page 2
	...
	--SKINS
		...
		skins_image:SetTexture([[Interface\AddOns\DamageMeterForever\images\icons2]])
		...
		-- window color
			...
			window_color:SetIcon ([[Interface\AddOns\DamageMeterForever\images\icons]], 14, 14, nil, {434/512, 466/512, 277/512, 307/512}, nil, 4, 2)
...
--numeral system page 3
		...
		sword_icon2:SetTexture([[Interface\AddOns\DamageMeterForever\images\icons2]])
...
-- stretcher
		...
		stretch_image:SetTexture([[Interface\AddOns\DamageMeterForever\images\icons]])
...
-- window button
		...
		instance_button_image:SetTexture([[Interface\AddOns\DamageMeterForever\images\icons]])
...
-- bookmark
		...
		shortcut_image2:SetTexture([[Interface\AddOns\DamageMeterForever\images\icons]])
		...
		bookmark_frame:SetBackdrop({bgFile = [[Interface\AddOns\DamageMeterForever\images\background]], tile = true, tileSize = 64})
		bookmark_frame:SetBackdropColor(0, 0, 0, 0.8)
		
		--local desc_anchor_topleft = _detalhes.gump:NewImage(bookmark_frame, [[Interface\AddOns\DamageMeterForever\images\options_window]], 75, 106, "artwork", {0.19921875, 0.2724609375, 0.6796875, 0.783203125}, "descAnchorBottomLeftImage", "$parentDescAnchorBottomLeftImage") --204 696 279 802
		--desc_anchor_topleft:SetPoint("topleft", bookmark_frame, "topleft", -5, 5)

		--local desc_anchor_bottomleft = _detalhes.gump:NewImage(bookmark_frame, [[Interface\AddOns\DamageMeterForever\images\options_window]], 75, 106, "artwork", {0.2724609375, 0.19921875, 0.783203125, 0.6796875}, "descAnchorTopLeftImage", "$parentDescAnchorTopLeftImage") --204 696 279 802
...
-- group windows
		...
		snap_image1:SetTexture([[Interface\AddOns\DamageMeterForever\images\icons]])
...
-- tooltip shift alt ctrl
		...
		micro_image1:SetTexture([[Interface\AddOns\DamageMeterForever\images\icons]])
...
--page 11
		...
		plugins_image1:SetTexture([[Interface\AddOns\DamageMeterForever\images\icons2]])
...
---Compact Foofi splash for Classic Forever (replaces stock Details wizard).
function Details:OpenDamageForeverWelcome()
	if (_G.DamageForeverWelcome) then
		_G.DamageForeverWelcome:Show()
		return
	end

	local ACCENT_R, ACCENT_G, ACCENT_B = 12 / 255, 210 / 255, 157 / 255
	local window = Details:CreateWelcomePanel("DamageForeverWelcome", UIParent, 480, 220, true)
	window:SetPoint("center", UIParent, "center", 0, 40)
	window:SetBackdropColor(0.02, 0.05, 0.04, 0.95)
	window:SetFrameStrata("FULLSCREEN_DIALOG")

	local accent = window:CreateTexture(nil, "BORDER")
	accent:SetPoint("topleft", window, "topleft", 1, -1)
	accent:SetPoint("topright", window, "topright", -1, -1)
	accent:SetHeight(3)
	accent:SetColorTexture(ACCENT_R, ACCENT_G, ACCENT_B, 1)

	local logo = window:CreateTexture(nil, "OVERLAY")
	logo:SetTexture([[Interface\AddOns\DamageMeterForever\images\damageforever_logo]])
	logo:SetSize(300, 94)
	logo:SetPoint("top", window, "top", 0, -18)

	local body = window:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
	body:SetPoint("top", logo, "bottom", 0, -8)
	body:SetWidth(420)
	body:SetJustifyH("CENTER")
	body:SetTextColor(0.9, 0.95, 0.93, 1)
	body:SetText("Foofi damage meter for Classic Forever.\nRight-click the window for modes · gear for options · |cffffff00/df|r")

	local close = CreateFrame("Button", nil, window, "UIPanelButtonTemplate")
	close:SetSize(120, 24)
	close:SetPoint("bottom", window, "bottom", 0, 18)
