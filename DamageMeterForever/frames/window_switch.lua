		...
		if (not allDisplaysFrame.already_built) then
			...
			for attribute = 1, Details.atributos[0] do
				...
				for i = 1, #Details.sub_atributos [attribute].lista do
					...
					local canAdd = (isApoc and type(damageMeterType) == "number" and damageMeterType < 100) or (not isApoc)
			...
			title_icon:SetTexture([[Interface\AddOns\DamageMeterForever\images\icons]])
			...
			--prepare for plugins
				...
				local title_icon = allDisplaysFrame:CreateTexture(nil, "overlay")
				title_icon:SetTexture([[Interface\AddOns\DamageMeterForever\images\modo_icones]])
		...
		if (options and options.sub_atributo) then
			if (options.atributo == 5) then --custom
				...
				if (not CustomObject) then --ele j� foi deletado
					icone = [[Interface\AddOns\DamageMeterForever\images\icons]]
					coords = add_coords
