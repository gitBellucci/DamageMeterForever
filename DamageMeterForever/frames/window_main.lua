...
local defaultBackdropSt = {
	bgFile = [[Interface\AddOns\DamageMeterForever\images\background]], tile = true, tileSize = 12,
...
--skins TCoords
	local DEFAULT_SKIN = [[Interface\AddOns\DamageMeterForever\images\skins\classic_skin]]
	...
	local menus_backdrop = {
		...
		edgeSize=1,
		bgFile = [[Interface\AddOns\DamageMeterForever\images\background]],
		tileSize=16,
		tile=true,
...
local update_line = function(self, target_frame)
	...
	for i = 0, (distance/pixelsPerArrow) do
		...
		if (not ball) then
			...
			ball:SetTexture([[Interface\AddOns\DamageMeterForever\images\icons]])
			ball:SetSize(16, 16)
			ball:SetAlpha(0.3)
...
local movement_onupdate = function(self, elapsed)
		...
		if (instance_ids_shown and instance_ids_shown > 0.95) then
			...
			if (need_show_group_guide and not DetailsFramework.IsTimewalkWoW()) then
				if Details.MicroButtonAlert then
					if Details.MicroButtonAlert.Text then
...
local resizeTooltip = {
	...
	{icon = [[Interface\AddOns\DamageMeterForever\images\key_shift]], width = 24, height = 14, l = 0, r = 1, t = 0, b =0.640625},

	{text = "+|cff33CC00 Click|cffEEEEEE: " .. Loc["STRING_RESIZE_VERTICAL"]},
	{icon = [[Interface\AddOns\DamageMeterForever\images\key_alt]], width = 24, height = 14, l = 0, r = 1, t = 0, b =0.640625},

	{text = "+|cff33CC00 Click|cffEEEEEE: " .. Loc["STRING_RESIZE_ALL"]},
	{icon = [[Interface\AddOns\DamageMeterForever\images\key_ctrl]], width = 24, height = 14, l = 0, r = 1, t = 0, b =0.640625}
}
...
local unSnapButtonTooltip = {
	...
	{icon = [[Interface\AddOns\DamageMeterForever\images\icons]], width = 14, height = 14, l = 160/512, r = 179/512, t = 142/512, b = 162/512},
}
	...
	if (actor) then
		...
		elseif (actor.nome) then --ensure it's an actor table
			...
			if (class == "UNKNOW" or class == "UNGROUPPLAYER") then
				GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\classes_small_alpha]], 1, 1, iconSize, iconSize, 0, 0.25, 0.75, 1)
			else
				GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\classes_small_alpha]], 1, 1, iconSize, iconSize, classL, classR, classT, classB)
			end

			if (specL) then
				GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\spec_icons_normal_alpha]], 1, 2, iconSize, iconSize, specL, specR, specT, specB)
			else
			...
			GameCooltip:SetOption("StatusBarTexture", [[Interface\AddOns\DamageMeterForever\images\bar_skyline]])
...
local function CreateAlertFrame(baseframe, instancia)
	...
	alertBackgroundFrame:SetBackdrop({bgFile = [[Interface\AddOns\DamageMeterForever\images\background]], tile = true, tileSize = 16,
	insets = {left = 0, right = 0, top = 0, bottom = 0}})
...
do
	...
	icon:SetTexture([[Interface\AddOns\DamageMeterForever\images\minimap]])
	icon:SetPoint("left", tooltipAnchor, "left", 4, 0)
	icon:SetSize(18, 18)

	local text = tooltipAnchor:CreateFontString(nil, "overlay")
...
function gump:CriaJanelaPrincipal(ID, instancia, criando)
	...
	titleBar.texture:SetTexture([[Interface\AddOns\DamageMeterForever\images\bar_serenity]])
...
-- side bars highlights ------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	--top
		...
		instancia.h_cima:SetTexture([[Interface\AddOns\DamageMeterForever\images\highlight_updown]])
	...
	--bottom
		...
		instancia.h_baixo:SetTexture([[Interface\AddOns\DamageMeterForever\images\highlight_updown]])
	...
	--left
		...
		instancia.h_esquerda:SetTexture([[Interface\AddOns\DamageMeterForever\images\highlight_leftright]])
	...
	--right
		...
		instancia.h_direita:SetTexture([[Interface\AddOns\DamageMeterForever\images\highlight_leftright]])
...
function gump:CreateNewLine(instance, index)
	...
	--newLine.extraStatusbar.texture:SetTexture([[Interface\AddOns\DamageMeterForever\images\bar_textures\bar_of_bars.png]]) --setColorTexture is very expensive, so set the color once and use vertex color to change it
...
function gump:CriaRodape(baseframe, instancia)
	...
	bgFile = [[Interface\AddOns\DamageMeterForever\images\background]], tile = true, tileSize = 16,
	insets = {left = 0, right = 0, top = 0, bottom = 0}})
...
local build_mode_list = function(self, deltaTime)
	...
	if (parameters_table[2] > 0.15) then
		...
		gameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\modo_icones]], 1, 1, 20, 20, 32/256, 32/256*2, 0, 1)

		gameCooltip:AddLine(Loc["STRING_MODE_ALL"])
		gameCooltip:AddMenu(1, function() instance:SetMode(3) end)
		gameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\modo_icones]], 1, 1, 20, 20, 32/256*2, 32/256*3, 0, 1)

		gameCooltip:AddLine(Loc["STRING_OPTIONS_PLUGINS"])
		gameCooltip:AddMenu(1, function() instance:SetMode(4) end)
		gameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\modo_icones]], 1, 1, 20, 20, 32/256*3, 32/256*4, 0, 1)
		...
		gameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\modo_icones]], 1, 1, 20, 20, 0.625, 0.75, 0, 1)
		...
		if not detailsFramework.IsAddonApocalypseWow() then
			...
			gameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\assets\textures\icons\midnight.png]], 2, 1, 16, 14)
			if (hasClosedInstances) then
				GameCooltip:AddLine("$div", nil, 2, nil, -5, -11)
			end
		end
		...
		gameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\images\modo_icones]], 1, 1, 20, 20, 0.5, 0.625, 0, 1)
			...
			if (thisCombat.is_mythic_dungeon_segment) then
				...
				if (mythicDungeonInfo) then
					...
					if (isMythicOverallSegment) then
						...
						--CoolTip:AddIcon([[Interface\AddOns\DamageMeterForever\images\icons]], "main", "left", 14, 10, 479/512, 510/512, 24/512, 51/512)
						gameCooltip:AddLine(zoneName .. " +" .. mythicLevel .. " (" .. Loc["STRING_SEGMENTS_LIST_OVERALL"] .. ")", nil, 2, "white", "white")
...
function Details:ChangeSkin(skin_name)
	...
	do
		...
		if (not toolbar_icon_file) then
			toolbar_icon_file = [[Interface\AddOns\DamageMeterForever\images\toolbar_icons]]
		end
...
local reportButton_OnEnter = function(self, motion, forced)
	...
	GameCooltip:AddIcon([[Interface\AddOns\DamageMeterForever\Images\report_button]], 1, 1, 12, 19)
...
function gump:CriaCabecalho (baseframe, instancia)
	...
	baseframe.cabecalho.fechar:SetNormalTexture([[Interface\AddOns\DamageMeterForever\images\toolbar_icons]])
	baseframe.cabecalho.fechar:GetNormalTexture():SetTexCoord(160/256, 192/256, 0, 1)
	baseframe.cabecalho.fechar:SetHighlightTexture([[Interface\AddOns\DamageMeterForever\images\toolbar_icons]])
	baseframe.cabecalho.fechar:GetHighlightTexture():SetTexCoord(160/256, 192/256, 0, 1)
	baseframe.cabecalho.fechar:SetPushedTexture([[Interface\AddOns\DamageMeterForever\images\toolbar_icons]])
...
--title bar buttons
	...
	baseframe.cabecalho.modo_selecao = gump:NewButton(baseframe, nil, "DetailsModeButton"..instancia.meu_id, nil, 16, 16, modeSelector_OnClick, nil, nil, [[Interface\AddOns\DamageMeterForever\images\modo_icone]])
	...
	b:SetNormalTexture([[Interface\AddOns\DamageMeterForever\images\toolbar_icons]])
	b:GetNormalTexture():SetTexCoord(0/256, 32/256, 0, 1)
	b:SetHighlightTexture([[Interface\AddOns\DamageMeterForever\images\toolbar_icons]])
	b:GetHighlightTexture():SetTexCoord(0/256, 32/256, 0, 1)
	b:SetPushedTexture([[Interface\AddOns\DamageMeterForever\images\toolbar_icons]])
	...
	baseframe.cabecalho.segmento = gump:NewButton(baseframe, nil, "DetailsSegmentButton"..instancia.meu_id, nil, 16, 16, segmentSelector_OnClick, nil, nil, [[Interface\AddOns\DamageMeterForever\images\segmentos_icone]])
	...
	reportButton:SetNormalTexture([[Interface\AddOns\DamageMeterForever\images\toolbar_icons]])
	reportButton:GetNormalTexture():SetTexCoord(96/256, 128/256, 0, 1)
	reportButton:SetHighlightTexture([[Interface\AddOns\DamageMeterForever\images\toolbar_icons]])
	reportButton:GetHighlightTexture():SetTexCoord(96/256, 128/256, 0, 1)
	reportButton:SetPushedTexture([[Interface\AddOns\DamageMeterForever\images\toolbar_icons]])
	...
	resetButton:SetNormalTexture([[Interface\AddOns\DamageMeterForever\images\toolbar_icons]])
	resetButton:GetNormalTexture():SetTexCoord(128/256, 160/256, 0, 1)

	resetButton:SetHighlightTexture([[Interface\AddOns\DamageMeterForever\images\toolbar_icons]])
	resetButton:GetHighlightTexture():SetTexCoord(128/256, 160/256, 0, 1)

	resetButton:SetPushedTexture([[Interface\AddOns\DamageMeterForever\images\toolbar_icons]])
