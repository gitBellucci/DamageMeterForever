...
local refreshFunc = function(self, data, offset, totalLines)
    ...
    if type(playerData) ~= "table" then
        -- Scroll refreshes during layout before GeneratePlayerData has run
        -- (Forever / empty C_DamageMeter session). Bail out cleanly.
        return
    end
...
function breakdownMidnight.PlayerSectionInit(sectionFrame, windowFrame)
    ...
    playerScroll.RefreshMe = function(thisPlayerScroll)
        ...
        if type(playerData) ~= "table" then
            playerData = Details222.B.GetEmptySegment()
        end
        if type(playerData.combatSources) ~= "table" then
            playerData.combatSources = {}
        end
