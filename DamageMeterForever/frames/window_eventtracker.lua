...
function Details:CreateEventTrackerFrame(parentObject, name)
	...
	--> scroll frame
		...
		local get_player_icon = function(spec, class)
			...
			elseif (class) then
				return [[Interface\AddOns\DamageMeterForever\images\classes_small]], unpack(Details.class_coords [class])
			else
				return [[Interface\AddOns\DamageMeterForever\images\classes_plus]], 0.50390625, 0.62890625, 0, 0.125
			end
		end
			...
			for i = 1, total_lines do
				...
				if (ability) then
					...
					if (ability [ABILITYTABLE_SPELLTYPE] == SPELLTYPE_COOLDOWN) then
						...
						line.ActionIcon:SetTexture([[Interface\AddOns\DamageMeterForever\images\event_tracker_icons]])
