...
if detailsFramework.IsAddonApocalypseWow() then
    ...
    local function t(v)
        -- Missing enum members on Forever → treat as unsupported (100).
        return (type(v) == "number") and v or 100
    end
    displayMap = {
        [1] = {
            t(E.DamageDone), --damage done
            t(E.Dps), --dps
            t(E.DamageTaken), --damage taken
            100, --friendly fire (not supported)
            100, --frags (not supported)
            t(E.EnemyDamageTaken), --enemies
            100, --void zones (not supported)
            100, --damage taken by spells (not supported)
            t(E.AvoidableDamageTaken), -- avoidable damage taken
        },
        [2] = {
            t(E.HealingDone), --healing done
            t(E.Hps), --hps
            t(E.Absorbs), --absorbs (closest to overheal slot)
...
local onEvent = function(event, instance, ...)
    ...
    if event == "DETAILS_INSTANCE_CHANGEATTRIBUTE" then
        ...
        if bParser.IsDamageMeterSwapped() then
            ...
            if type(damageMeterType) == "number" and damageMeterType < 100 then
