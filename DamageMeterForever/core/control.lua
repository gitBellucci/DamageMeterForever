...
--internal functions
	...
	if detailsFramework.IsAddonApocalypseWow() then
		local getSegment = C_DamageMeter and C_DamageMeter.GetCombatSessionFromType
		local serverInCombat = false
		if getSegment then
			-- Current session + DamageDone. In combat, totals are secret (same as Epic/Luckyone).
			local sessionType = Enum.DamageMeterSessionType and Enum.DamageMeterSessionType.Current or 1
			local meterType = Enum.DamageMeterType and Enum.DamageMeterType.DamageDone or 0
			C_Timer.NewTicker(0.2, function()
				local thisSegment = getSegment(sessionType, meterType)
				if thisSegment then
					local amount = thisSegment.totalAmount
					if amount == nil then
						amount = thisSegment.maxValue
					end
	...
	function Details:AddTooltipBackgroundStatusbar_Secret(value, maxValue, useSpark, statusBarColor)
		...
		--GameCooltip:AddStatusBar(value, 1, r, g, b, a, useSpark, {value = 100, color = {rBG, gBG, bBG, aBG}, texture = [[Interface\AddOns\DamageMeterForever\images\bar_serenity]]})
		GameCooltip:AddStatusBar_MaxValue(value, maxValue, 1, r, g, b, a, useSpark, {value = 100, color = {rBG, gBG, bBG, aBG}, texture = [[Interface\AddOns\DamageMeterForever\images\bar_serenity]]})
	end
...
-- /run local a,b=Details.tooltip.header_statusbar,0.3;a[1]=b;a[2]=b;a[3]=b;a[4]=0.8;
	...
	local backdrop = {bgFile = [[Interface\AddOns\DamageMeterForever\images\background83.png]], edgeFile = [[Interface\Buttons\WHITE8X8]], tile=true,
	...
	function Details:FormatCooltipForSpells()
		...
		GameCooltip:SetOption("StatusBarTexture", [[Interface\AddOns\DamageMeterForever\images\bar_background_dark_withline]])
		--GameCooltip:SetOption("StatusBarTexture", [[Interface\AddOns\DamageMeterForever\images\bar_textures\bar_rounded.png]])
	...
	function Details:PostBuildInstanceBarTooltip(actorObject)
		...
		if (not Details.GameCooltipFrame1Shadow) then
			...
			Details.GameCooltipFrame1Shadow:SetTexture([[Interface\AddOns\DamageMeterForever\images\shadow_square.png]], nil, nil, "TRILINEAR")
